	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			if ((i - m->nmaster) % 2 ) {
				...
				resize(c, m->wx + gappx/ns, m->wy + ety, tw - (2*c->bw) - gappx * (5-ns)/2,
						h - (2*c->bw), 0);
				ety += HEIGHT(c) + gappx;
			} else {
...
centeredfloatingmaster(Monitor *m)
{
	...
	for(i = tx = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + mx + gappx, m->wy + my, w - (2*c->bw) - gappx * (5-ns)/2,
					mh - (2*c->bw), 0);
			mx += WIDTH(c) + gappx;
		} else {
			/* stack clients are stacked horizontally */
			w = (m->ww - tx) / (n - i) - gappx;
			resize(c, m->wx + tx + gappx/ns, m->wy, w - (2*c->bw) - gappx * (5-ns)/2,
					m->wh - (2*c->bw), 0);
			tx += WIDTH(c) + gappx;
		}
}
