...
void next(const Arg *arg) {
  int nxt = selmon->tagset[selmon->seltags];
  // nxt == TAGMASK -> All Mode
  if (!(nxt ^ TAGMASK))
    return;
  if (arg->ui) {
    // ui != 0 -> Next (Right)
  ...
  selmon->seltags ^= 1;
  selmon->tagset[selmon->seltags] = nxt & TAGMASK;
  focus(NULL);
  arrange(selmon);
}
...
void centeredmaster(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
       ...
       * of the screen */
      h = (m->wh - my) / (MIN(n, m->nmaster) - i) - gappx;
      resize(c, m->wx + mx + gappx, m->wy + my,
             mw - (2 * c->bw) - gappx * (5 - ns) / 2, h - (2 * c->bw), 0);
      my += HEIGHT(c) + gappx;
    } else {
      /* stack clients are stacked vertically */
      if ((i - m->nmaster) % 2) {
        h = (m->wh - ety) / ((1 + n - i) / 2) - gappx;
        resize(c, m->wx + gappx / ns, m->wy + ety,
...
void centeredfloatingmaster(Monitor *m) {
  unsigned int i, n, w, mh, mw, mx, mxo, my, myo, tx, ns;
  Client *c;
  ...
  for (i = tx = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
       ...
       * of the screen */
      w = (mw + mxo - mx) / (MIN(n, m->nmaster) - i) - gappx;
      resize(c, m->wx + mx + gappx, m->wy + my,
             w - (2 * c->bw) - gappx * (5 - ns) / 2, mh - (2 * c->bw), 0);
      mx += WIDTH(c) + gappx;
    } else {
      /* stack clients are stacked horizontally */
      w = (m->ww - tx) / (n - i) - gappx;
      resize(c, m->wx + tx + gappx / ns, m->wy,
             w - (2 * c->bw) - gappx * (5 - ns) / 2, m->wh - (2 * c->bw), 0);
      tx += WIDTH(c) + gappx;
    }
}
